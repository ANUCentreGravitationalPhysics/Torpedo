...
% Parameters for the main interferometer (others are kept to their nominal value)
  ...
  ifo.Optics.SRM.TransmittanceD = [   0.25,   0.0005];  % SRM transmittance for [carrier A, carrier B]
...
% Parameters for the input filter cavity
  ...
  ifo.InputFilterCavity.Rim     = [0.999815,0.999815]; % Input mirror refltectivity for [A, B] [power]
  ifo.InputFilterCavity.loss    = [   32e-6,   32e-6]; % Power loss in the filter cavity for [A, B]
  ifo.InputFilterCavity.detune  = [     20.7,    100]; % Detuned frequency [Hz] for [A, B]  
...
% Plot Confidence interval for the output filter cavity losses
 ...
 qs = [];
 Looos = ifo.Laser.PBSD(2)*[1 0.000001];
 for kk = 1:length(Looos)
     ifo.Laser.PBSD(2) = Looos(kk);
...
title('AdvLIGO with local readout and 100m input filter')
