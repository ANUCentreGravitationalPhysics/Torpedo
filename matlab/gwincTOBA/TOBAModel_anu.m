...
function ifo = TOBAModel_anu(varargin)

  ifo.Bar.Length                = 1.3; % m
  ifo.Bar.Radius                = 0.095; % m
  ifo.Bar.Substrate.Material    = 'Aliminium';
  

  ifo.Bar.Suspension.Material   = 'Aliminium';
  ifo.Bar.Suspension.SafetyFactor = 1;                    % safety factor under breaking strength of wires
  ifo.Bar.Suspension.Length     = 0.9;
  ifo.Bar.Suspension.WireRadius = NaN;                  % to be calculated in suspTorsion
  ifo.Bar.Suspension.NWire      = 2;

  %% Infrastructure----------------------------------------------------------
  
  ifo.Infrastructure.Length                     = ifo.Bar.Length / sqrt(2);      % m;
  ...
  ifo.Seismic.Omicron = 1e0;                     % Feedforward cancellation factor
  ifo.Atmospheric.Omicron = 1e2;                  % Feedforward atmospheric NN cancellation factor
  ...
  ifo.Suspension.Type         = 'Torsion';%'Single';             % 0 for cylindrical suspension
  ...
  ifo.Materials.Silicon.MassCM = 0.3;                  % J/Kg/K; specific heat (ref. 4)
  ...
  % Aluminium
  ifo.Materials.Aluminium.c2  = 7.6e-10;                 % Coeff of freq depend. term for bulk mechanical loss, 7.15e-12 for Sup2
  ifo.Materials.Aluminium.MechanicalLossExponent=0.77;   % Exponent for freq dependence of silica loss, 0.822 for Sup2
  ifo.Materials.Aluminium.Alphas = 5.2e-10;              % Surface loss limit (ref. 12)
  ifo.Materials.Aluminium.MirrorY    = 69e9;            % N/m^2; Youngs modulus (ref. 17)
  ifo.Materials.Aluminium.MirrorSigma = 0.35;            % Kg/m^3; Poisson ratio (ref. 17)
  ifo.Materials.Aluminium.MassDensity = 2729;            % Kg/m^3; (ref. 17)
  ifo.Materials.Aluminium.MassAlpha = 1e-9;             % 1/K; nominally zero
  ifo.Materials.Aluminium.MassCM = 0.897;                  % J/Kg/K; specific heat (ref. 4)
  ifo.Materials.Aluminium.MassKappa = 237;              % J/m/s/K; thermal conductivity (ref. 4)
  ifo.Materials.Aluminium.G         = 25e9;              % shear modulus
  ifo.Materials.Aluminium.Tensile   = 483e6;               % Ultimate Tensile strength, Pa
  ...
  ifo.Bar.Substrate = ifo.Materials.Aluminium;
  ...
  % ifo.Mirror.Substrate = ifo.Materials.FusedSilica;
  ...
  ifo.Optics.ITM.Thickness = 0.25 * 25.4e-3; %ifo.Materials.MassThickness;
  ifo.Optics.ETM.Thickness = 0.25 * 25.4e-3; %ifo.Materials.MassThickness;   % not used, BS
  
  ifo.Optics.MirrorRadius = 1 *25.4e-3; %ifo.Materials.MassRadius;             % 2" mirrors, used in subbrownianFiniteCorr, BS
  ...
  ifo.Optics.coupling = 0.0;                   % mismatch btwn arms & SRC modes; used to
                                                ...
                                                % r_srm, BS
  ifo.Optics.Curvature.ITM = 2;               % ROC of ITM
  ifo.Optics.Curvature.ETM = 2;               % ROC of ETM
  ...
  % 
  ifo.Optics.ITM.BeamRadius = 0.00061;                     % m; 1/e^2 power radius
  ifo.Optics.ETM.BeamRadius = 0.00061;                     % m; 1/e^2 power radius
  ...
  ifo.Optics.PRM.Transmittance  = 1;                   %Transmision of PRM, 0.03;
