...
%   wp =   sqrt(g / sqrt(ifo.Bar.Suspension.Length));    % pendulum frequency in rad
%   phip = 1e-5;
%   
%   Xp = 1/mb ./ (wp^2*(1 + i*phip) - w.^2);   % pendulum  response
...
function [nx, np] = MinusK(f, ifo)
  ...
  noisefloor = 10.*anu_seismometer(f);
