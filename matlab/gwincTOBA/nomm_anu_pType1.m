...
systemFilename.base = 'anuTobaPrototype0p6m_AluBar_TungstenSus_MultiSas10pm';
...
%% --  Small Design mods ---
  %
  ifo.Laser.Power                        = 0.1; % W;
  ifo.Laser.Wavelength                   = 1064e-9; % m;

  % Bar and Suspension Material

%% Bar mass of ~10kg
  ifo.Bar.Suspension.Length     = 0.6 + 0.085;  % m
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
  ...
  ifo.Bar.Suspension.WireRadius = 0.00025;                   % minimum wire radius, with a safety factor
...
% Suspensoin environment  
  ...
  ifo.Seismic.isolationType = 'MinusK';       % 'BSC' (aLIGO ISI BSC requirement), 
  ...
  elseif strcmp(ifo.Seismic.isolationType, 'MinusK')
        ifo.Seismic.isolationComment = 'ground x MinusK w/ ANU-Sensor';
  ...
  elseif strcmp(ifo.Seismic.isolationType, 'ANUP')
        ifo.Seismic.isolationComment = 'sus point at 100mp';
  else
...
%figure('Name','TORPEDO Total Noise Budget')
