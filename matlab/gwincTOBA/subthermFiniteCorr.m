...
% 227-236 (hereafter BHV) or Liu and Thorne gr-qc/0002055 (hereafter LT)
  ...
  a = ifo.Optics.MirrorRadius;
  h = ifo.Optics.MirrorThickness;
...
%   % newly rewritten LT 46, BS
%   coeff = (1-Qm).^2 .* (1+Qm) + 8*h*km.*Qm.*(1-Qm) + 4*h^2 * km.^2 .* Qm .* (1+Qm);
%   coeff = coeff .* (a^5 * km .* pm.^2 .* (1-Qm).*j0m.^2) ./ ((1-Qm).^2 - 4*h^2 * km.^2 .* Qm).^2;
%   coeff = sum(coeff);
%   coeff = coeff + (a^5 * h *c1^2) / (1+sigma)^2;
%   coeff = coeff * (2*pi)^(3/2) * r0^3 / a^3;
