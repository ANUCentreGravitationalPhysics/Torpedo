...
save_figure = 1;        % flag to save the figures and output parameters
...
%% Bar mass of ~10kg
  ...
  ifo.Suspension.vForce = zeros(size(Experiment.Freq'));
  ...
  ifo.Suspension.vTable = zeros(size(Experiment.Freq'));
...
%% Torpedo Bar Inertia and Mass Parameters
%  
%     % Bar X parameters
...
%     ifo.Bar.Inertia = (ifo.Bar.X.I(3,3) + ifo.Bar.Y.I(3,3)) /2;
  ...
  % Bar mass, kg
  ifo.Bar.Mass = Experiment.Bar(2).Mass;

  % rotational inertia, of a single bar
  ifo.Bar.Inertia = Experiment.Bar(1).I(3,3);  

  % moment of inertia along the axis of the bar
  ifo.Bar.Ipit = Experiment.Bar(2).I(1,1);      
...
%% Cavity Mirror Parameters  
  ...
  % used in shotradSignalRecycling,
  ifo.Materials.Substrate.Mass = ifo.Bar.Mass;      
  
  % set the IFO mirror materials parameters
  ifo.Materials.Substrate = ifo.Materials.(ifo.Optics.Substrate);   

  % Cavity Mirror Substrate absorption
  ...
  % Calculate the Cavity waist size and location
...
%   ifo.Optics.Arm.zetm = z2;
  ...
  [ifo.Optics.Arm.waist, ...
   ifo.Optics.Arm.z0, ...
   ifo.Optics.Arm.zitm, ...
   ifo.Optics.Arm.zetm] = cwaist(ifo.Optics.ITM.gfactor, ...
                                 ifo.Optics.ETM.gfactor, ...
                                 ifo.Optics.Arm.CavityLength, ...
                                 ifo.Laser.Wavelength);
  ...
  ifo.Optics.ITM.BeamRadius = spotsize(ifo.Optics.Arm.waist, ...
                                       ifo.Optics.Arm.z0, ifo.Optics.Arm.zitm);                     
  ifo.Optics.ETM.BeamRadius = spotsize(ifo.Optics.Arm.waist, ...
                                       ifo.Optics.Arm.z0, ifo.Optics.Arm.zetm);                     
...
%% Suspension Point Displacement Setting  
  ...
  ifo.Seismic.isolationType = 'T240';   
  ...
  elseif strcmp(ifo.Seismic.isolationType, 'T240')
        ifo.Seismic.isolationComment = 'T240 noisefloor';
