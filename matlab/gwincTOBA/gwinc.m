...
%f = logspace(log10(flo), log10(fhi), 6000);
f = ifo.Experiament.Freq';
...
switch ifo.Suspension.Type
  ...
  case {0, 1}
    [hTorque, vTorque, yTable, vTable] = suspQuad(f, ifo);
  case 'Quad'
...
%     fname = 'suspTorsion';
    fname = 'suspTorpedo3';
    disp(['Torsion Suspension (', fname,'.m)']);
    %disp(ifo.Suspension.Type)
    [hForce, vForce, hTable, vTable] = feval(fname, f, ifo);
%     [hTorque, vTorque, yTable, vTable] = feval(fname, f, ifo);
...
% Loaded via load '../../Mathematica/protoType/ComplexV8/TorpedoV8.mat';
%ifo.Suspension.hForce = hForce;
%ifo.Suspension.vForce = vForce;
% ifo.Suspension.hTorque = hTorque;
% ifo.Suspension.vTorque = vTorque;

%ifo.Suspension.hTable = hTable;
%ifo.Suspension.vTable = vTable;
% ifo.Suspension.yTable = yTable;
% ifo.Suspension.vTable = vTable;
...
case 0
 ...
 y9  = gravg4(f,ifo);                           % Seismic Gravity Gradients
 y10 = seismic(f,ifo);                         % Seismic
 y11 = thermooptic(f,ifo);                     % coating thermo-optic (TE + TR)
 y12 = subtoba(f,ifo);                         % Toba brownian noise
% y13 = atmgravg(f, ifo);                       % atmospheric GG
 y13 = atmgravg2(f,ifo);                       % atmospheric GG
 y15 = atmgravg3(f,ifo);                        % atmopheric GG due to wind speed
 y14 = laserfrequency(f, ifo);                 % calculates the laser frequency noise wrt the CM Arms
...
%ys = y1 + y2 + y3 + y4 + y9 + y10 + y12 + y13;       % sum of noise sources
ys = y1 + y2 + y3 + y4 + y10 + y12;       % sum of noise sources ... NO GRAVG or ATMGRAV for low-freq PAPER

% whos y3 y4 y5 y6 y8 y9 y10 y11 y12 y13 y2
...
if (fig ~= 0)
  ...
  if ifo.Seismic.Omicron == 1,
      ...
      seismicNN = ['Seismic NN (' num2str(ifo.Seismic.Omicron,'%0.2d') 'x Cancellation)']
  end
  
  if ifo.Atmospheric.Omicron == 1,
      atmosphericNN = ['Infrasound Newtonian Noise, \deltaP'];
  else
      atmosphericNN = ['Infrasound NN, \deltaP (' num2str(ifo.Atmospheric.Omicron,'%0.2d') 'x Cancellation)'];
  end
