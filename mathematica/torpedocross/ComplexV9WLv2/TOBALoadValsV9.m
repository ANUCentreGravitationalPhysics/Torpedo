function [Experiament] = TOBALoadValsV9(CalFileNameRe, CalFileNameIm)
%Unpacks values from the mathmatica script into a structure in Matlab
%   [Experiament] = TOBALoadVals(CalFileName)

% Unpacking the complex calibration numbers from Mathmatica
ValsRe = dlmread(CalFileNameRe);
ValsIm = dlmread(CalFileNameIm);
Vals = ValsRe + 1i.*ValsIm;
...
for i = BarDexs
    ...
    Experiament.Bar(i).RestWirePitch = Vals(IndexStart+23);
    Experiament.Bar(i).RestWireRoll = Vals(IndexStart+24); 
    Experiament.Bar(i).RestBarRoll = Vals(IndexStart+25); 
    Experiament.Bar(i).RestBarPitch = Vals(IndexStart+26);
    Experiament.Bar(i).RestBarYaw = Vals(IndexStart+27);
end
